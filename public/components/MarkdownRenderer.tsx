  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h3: ({ ...props }) => <h3 className="text-lg md:text-xl " {...props} />,
        h4: ({ ...props }) => (
        ...
        p: ({
          ...
          const tag =
            first?.type === "element"
              ? (first as unknown as Element).tagName
              : undefined;
          if (
            isOnlyBlock &&
        ...
        code: ({
          ...
          if (isBlock) {
            return (
              <NoirCodeBlock language={language || "code"} {...props}>
                {children}
              </NoirCodeBlock>
            );
          }
...
type NoirCodeBlockProps = ComponentPropsWithoutRef<"code"> & {
  language: string;
  children: ReactNode;
};

export function NoirCodeBlock({
  language,
  children,
  ...props
}: NoirCodeBlockProps) {
